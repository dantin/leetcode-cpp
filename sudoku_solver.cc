...
class Solution {
 ...
 private:
  void dfs(vector<vector<char>> &board, vector<vector<bool>> &rows,
           vector<vector<bool>> &columns, vector<vector<bool>> &boxes,
           vector<pair<size_t, size_t>> &spaces, size_t pos, bool &done) {
    if (pos == spaces.size()) {
      done = true;
      return;
    }
