...
def scrape_problems(cookies: dict[str, str]):
    ...
    for item in stat_status_pairs:
        ...
        problems.append(Problem(pid=pid, title=title, name=slug, url=href, difficulty=difficulty, status=status))
...
def generate_problemset(problems: list[Problem], root_dir: Path):
    ...
    for p in problems:
        ...
        lines.append(f"| {p.pid} | {name_link} | {p.difficulty} | {p.status} | {sol_link} |")
...
if __name__ == "__main__":
    ...
    fetch_parser = subparsers.add_parser("fetch", help="Fetch and persist a specific problem")
    fetch_parser.add_argument("problem_id", type=int, help="The ID of the problem to fetch")
    
    problems_parser = subparsers.add_parser("problems", help="Retrieve all problems and update PROBLEMSET.md")
