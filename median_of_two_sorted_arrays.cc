...
class Solution {
 public:
  double findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2) {
    ...
    if ((len1 + len2) % 2 == 0) {
      double r1 =
          (double)findKth(nums1, 0, len1, nums2, 0, len2, (len1 + len2) / 2);
      double r2 = (double)findKth(nums1, 0, len1, nums2, 0, len2,
                                  (len1 + len2) / 2 + 1);
      return (r1 + r2) / 2.0;
    } else {
 ...
 private:
  int findKth(vector<int> &nums1, size_t begin1, size_t end1,
              vector<int> &nums2, size_t begin2, size_t end2, size_t k) {
    ...
    if (nums1[mid1] <= nums2[mid2]) {
      if (n / 2 + m / 2 + 1 >= k) {
        ...
        return findKth(nums1, mid1 + 1, end1, nums2, begin2, end2,
                       k - n / 2 - 1);
      }
      ...
      if (n / 2 + m / 2 + 1 >= k) {
        ...
        return findKth(nums1, begin1, end1, nums2, mid2 + 1, end2,
                       k - m / 2 - 1);
      }
    }
  }
